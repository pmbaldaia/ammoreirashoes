...
const details: Record<string, string> = { pages: 'Páginas configuradas', products: 'Registos no catálogo', collections: 'Coleções publicadas', events: 'Feiras e eventos', contacts: 'Pedidos recebidos' }
...
const stats = rows.map(({ resource, items }) => ({ label: labels[resource], value: items.length, detail: details[resource], icon: icons[resource], tone: tones[resource] }))
...
<template>
  <div class="page-stack">
    <section class="page-heading">
      <div><p class="eyebrow">Visão geral</p><h1>Painel de controlo</h1><p class="page-heading__description">O centro de controlo do website AM Moreira.</p></div>
    ...
    <section class="dashboard-grid">
      <article class="panel quick-actions"><div class="panel__header"><div><h2>Ações rápidas</h2><p>Gerir conteúdo essencial</p></div></div><NuxtLink class="quick-action" to="/admin/produtos?new=1"><span><Icon name="lucide:package-plus" /></span><div><strong>Adicionar produto</strong><small>Catálogo informativo</small></div></NuxtLink><NuxtLink class="quick-action" to="/admin/eventos?new=1"><span><Icon name="lucide:calendar-plus" /></span><div><strong>Adicionar feira</strong><small>Agenda da marca</small></div></NuxtLink><NuxtLink class="quick-action" to="/admin/media"><span><Icon name="lucide:image-plus" /></span><div><strong>Carregar imagens</strong><small>Biblioteca de media</small></div></NuxtLink></article>
      <article class="panel"><div class="panel__header"><div><h2>Novos contactos</h2><p>Pedidos por analisar</p></div><NuxtLink class="text-button" to="/admin/contactos">Ver todos</NuxtLink></div><div v-if="newContacts.length" class="dashboard-contacts"><p v-for="item in newContacts.slice(0, 5)" :key="item.id"><strong>{{ item.name }}</strong> · {{ item.subject }}</p></div><div v-else class="empty-state"><h3>Tudo tratado</h3><p>Não existem novos pedidos.</p></div></article>
    </section>
  </div>
</template>
