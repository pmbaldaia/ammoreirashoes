...
<template>
  <div class="am-site">
    <template v-for="block in visibleBlocks" :key="block.id">
      ...
      <section v-else-if="block.type === 'events'" class="am-section"><div class="am-wrap"><div class="am-section__head"><div><p class="am-kicker">{{ block.eyebrow }}</p><h2>{{ block.title }}</h2></div><p>{{ block.content }}</p></div><div v-if="featuredEvents.length" class="am-grid"><article v-for="item in featuredEvents" :key="item.id" class="am-card"><div class="am-card__body"><p class="am-kicker">{{ item.city }} · {{ item.startDate }}</p><h3>{{ item.name }}</h3><p>{{ item.description }}</p></div></article></div></div></section>
